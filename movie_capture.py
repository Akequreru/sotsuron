...
def process_single_video(youtube_url):
    ...
    def contains_face(frame_data):
        ...
        frontal_faces = frontal_face_cascade.detectMultiScale(gray, 1.1, 15, minSize=(80, 80))
        if len(frontal_faces) > 0: return True
        profile_faces = profile_face_cascade.detectMultiScale(gray, 1.1, 15, minSize=(80, 80))
        if len(profile_faces) > 0: return True
        gray_flipped = cv2.flip(gray, 1)
        profile_flipped = profile_face_cascade.detectMultiScale(gray_flipped, 1.1, 15, minSize=(80, 80))
